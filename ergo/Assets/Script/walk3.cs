...
public class walk3 : MonoBehaviour
{
    ...
    private float steplong1 = 6f; // 已经移动的距离
    private float steplong2 = 13f; // 已经移动的距离
    private float steplong3 = 16f; // 已经移动的距离
    ...
    public Vector3 point3 = new Vector3(51f, 1.5f, 0f);
    ...
    void Update()
    {
        ...
        if ((Vector3.Distance(transform.position, FindClosestFire().transform.position) < 5 || Vector3.Distance(transform.position, FindSecondClosestFire().transform.position) < 5) && animator.GetBool("isRun") == false)
        {
            ...
            moveSpeed = 0.8f;
            agent.SetDestination(point3); // 设置导航目标为 point
